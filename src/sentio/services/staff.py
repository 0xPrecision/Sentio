...
from sentio.schemas.staff import ServiceCreate, ServiceUpdate, StaffMemberCreate, StaffMemberUpdate
...
class StaffMemberService:
    ...
    async def get_or_create_staff_member(
        self, tenant_id: int, data: StaffMemberCreate
    ) -> StaffMember:
        async with self.session.begin():
            staff_member = await self.staff_members.get_by_name_for_tenant(
                tenant_id=tenant_id, name=data.display_name
            )
    ...
    async def update_staff_member(
        self, staff_member: StaffMember, data: StaffMemberUpdate
    ) -> StaffMember:
        async with self.session.begin():
...
class ServiceManager:
    ...
    async def get_or_create_service(self, tenant_id: int, data: ServiceCreate) -> Service:
        async with self.session.begin():
    ...
    async def get_by_name_for_tenant(self, tenant_id: int, name: str) -> Service | None:
        return await self.services.get_by_name_for_tenant(tenant_id=tenant_id, name=name)

    async def list_by_tenant(self, tenant_id: int) -> list[Service]:
        return await self.services.list_by_tenant_id(tenant_id)

    async def update_service(self, service: Service, data: ServiceUpdate) -> Service:
        async with self.session.begin():
            update_data = data.model_dump(exclude_unset=True)
...
class StaffServices:
    ...
    async def get_or_create_relation(
        self, *, staff_member_id: int, service_id: int
    ) -> StaffService:
        async with self.session.begin():
            staff_service = await self.staff_services.get_by_service_and_staff_member_id(
                staff_member_id=staff_member_id, service_id=service_id
            )

            if staff_service is not None:
                return staff_service

            staff_service = StaffService(staff_member_id=staff_member_id, service_id=service_id)
